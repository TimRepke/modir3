...
if __name__ == '__main__':
    ...
    parser.add_argument('--graph-min-node-count', type=int,
                        help='Minimum number of times a node has to occur')
    parser.add_argument('--graph-max-node-count', type=int,
                        help='Maximum number of times a node can occur')
    parser.add_argument('--graph-min-node2doc-count', type=int,
                        help='Minimum number of times a node has to appear in one document to count as an occurrence')
    parser.add_argument('--graph-min-node2node-count', type=int,
                        help='Minimum edge weight between nodes')
    ...
    if cli_args.data_set == 'aminer':
        ...
        hypergraph = EnronHyperGraph(gensim_processor=gensim_processor, hnsw_tree=tree,
                                     file_name=cli_args.hypergraph_files,
                                     num_docs=gensim_processor.get_count(),
                                     input_dimensions=gensim_processor.get_dimensionality(),
                                     k_neighbourhood=cli_args.neighbourhood_k,
                                     ...
                                     min_num_mails=1, only_original=True)
    elif cli_args.data_set == 'news':
        hypergraph = NewsHyperGraph(gensim_processor=gensim_processor, hnsw_tree=tree,
                                    ...
                                    k_neighbourhood=cli_args.neighbourhood_k,
                                    k_global=cli_args.global_k,
                                    min_node_count=cli_args.graph_min_node_count,
                                    max_node_count=cli_args.graph_max_node_count,
                                    min_node2doc_count=cli_args.graph_min_node2doc_count,
                                    min_node2node_count=cli_args.graph_min_node2node_count)
    elif cli_args.data_set == 'mcc':
        raise AttributeError(f'There is no processing for {cli_args.data_set}!')
    else:
        raise AttributeError(f'There is no processing for {cli_args.data_set}!')
    ...
    LAYOUT_FILE = cli_args.data_out + '.pos'
    if not os.path.isfile(LAYOUT_FILE):
        time0 = timeit.default_timer()
        t = Trainer(hypergraph=hypergraph, learning_rate=0.01, related_samples=cli_args.related_k, epochs=50)
        t.train(intermediate_files=cli_args.data_out)
        t.model.save_embedding(LAYOUT_FILE)
    ...
    # WRITE EXPORT
    # ---------------------------------------------------
    print('----------------\n Writing MODiR interactive vis export...')
    time0 = timeit.default_timer()
    vis_exporter = None
    if cli_args.data_set == 'aminer':
        raise AttributeError(f'There is no processing for {cli_args.data_set}!')
    elif cli_args.data_set == 's2':
        vis_exporter = S2ModirVisExport(layout_file=LAYOUT_FILE, hypergraph=hypergraph,
                                        file_name=cli_args.data_out,
                                        word_grid_cols=10, word_grid_rows=10)
    elif cli_args.data_set == 'enron':
        vis_exporter = EnronModirVisExport(layout_file=LAYOUT_FILE, hypergraph=hypergraph,
                                           file_name=cli_args.data_out,
                                           word_grid_cols=10, word_grid_rows=10)
    elif cli_args.data_set == 'news':
        vis_exporter = NewsModirVisExport(layout_file=LAYOUT_FILE, hypergraph=hypergraph,
                                          file_name=cli_args.data_out,
                                          word_grid_cols=10, word_grid_rows=10)
    elif cli_args.data_set == 'mcc':
        raise AttributeError(f'There is no processing for {cli_args.data_set}!')
    ...
    vis_exporter.write_export()

    print(f'  - init done after: {timeit.default_timer() - time0:.4f}s')
    print(f'  - exported json vis file for {cli_args.data_set}')
