...
TARGET = '/home/tim/Uni/HPI/workspace/landscape_d3/data/news.json'

with open('../../data/news/coba_news_d2v.json', 'r') as f1, \
        open('../../data/news/coba_news_amb.json', 'r') as f2:
    vectors = []
    companies = {}
    documents = []

    doc_vectors = {}
    for line in tqdm(f1, desc='D2v Vector index'):
        doc = json.loads(line)
        doc_vectors[doc['url']] = doc['d2v']

    doc_cnt = 0
    tf_idf = []
    for doc_cnt, line in enumerate(tqdm(f2, desc='Extract docs and entities')):
        doc = json.loads(line)
        doc['d2v'] = doc_vectors[doc['url']]
